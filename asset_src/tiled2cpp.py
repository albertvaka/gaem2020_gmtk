...
current = Path('../engine/tiledexport.h').read_text()
if out_h == current:
    print("Skipping header generation, no changes detected")
else:
    Path('../engine/tiledexport.h').write_text(out_h)
...
current = Path('../engine/tiledexport.cpp').read_text()
if out_cpp == current:
    print("Skipping cpp generation, no changes detected")
else:
    Path('../engine/tiledexport.cpp').write_text(out_cpp)
