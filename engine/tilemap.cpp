...
void TileMap::Draw() const
{
	...
	RectToTextureCoordinates(Assets::invadersTexture, outOfBounds);
#endif
	...
	if (left < 0) {
		for (int y = top; y < bottom; y++)
		{
			...
			{
#ifdef USE_VAO
				Window::DrawRaw::BatchTexturedQuad(Assets::invadersTexture, x * Tile::size, y * Tile::size,16,16, outOfBounds);
#else
				Window::Draw(Assets::invadersTexture, vec(x * Tile::size, y * Tile::size))
	...
	for (int y = top; y < bottom; y++)
	{
		for (int x = left; x < right; x++)
		{
			...
			RectToTextureCoordinates(Assets::invadersTexture, rect);
			Window::DrawRaw::BatchTexturedQuad(Assets::invadersTexture, x * Tile::size, y * Tile::size, 16,16, rect);
#else
			Window::Draw(Assets::invadersTexture, vec(x * Tile::size, y * Tile::size))
...
#ifdef USE_VAO
	Window::DrawRaw::FlushTexturedQuads(Assets::invadersTexture);
#endif
}
